...
export function Ornament({ className = "" }: { className?: string }) {
  return (
    <div className={`flex items-center justify-center gap-4 ${className}`} aria-hidden="true">
      <span className="h-px w-16 bg-gradient-to-r from-transparent to-[var(--gold)]/45 sm:w-24" />
      <svg width="20" height="26" viewBox="0 0 24 32" fill="none" className="shrink-0">
        ...
        <path
          ...
          opacity="0.5"
        />
        <circle cx="12" cy="29" r="1.4" fill="var(--gold)" opacity="0.7" />
      </svg>
      <span className="h-px w-16 bg-gradient-to-l from-transparent to-[var(--gold)]/45 sm:w-24" />
    </div>
  );
}
