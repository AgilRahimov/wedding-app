  ...
  return (
    <form onSubmit={go} className="flex flex-col gap-3 sm:flex-row">
      <input
        ...
        className="flex-1 rounded-full border border-[var(--line)] bg-[var(--card)] px-5 py-3 text-[15px] text-[var(--ink)] outline-none transition placeholder:text-[var(--muted-2)] focus:border-[var(--gold)]/60"
      />
      <button
        type="submit"
        className="rounded-full bg-[var(--gold)] px-7 py-3 text-[13px] font-medium uppercase tracking-[0.16em] text-white transition hover:bg-[var(--gold-bright)]"
      >
        Open
      </button>
    </form>
