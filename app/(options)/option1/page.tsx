...
export const metadata: Metadata = { title: "Option 1 · Ivory" };

// Option 1 — "Ivory": the first light version, kept for comparison.
const PALETTE = `
  .opt1 {
          --paper: #f6f1e7;        /* page background — warm ivory paper */
          --card: #fcfaf5;         /* raised cards, a shade lighter than the paper */
          --ink: #4a3a26;          /* body text — warm dark brown */
          --ink-strong: #2c2216;   /* headings */
          --muted: #7a6a52;        /* secondary text */
          --muted-2: #a3957f;      /* captions, hints */
          --line: rgba(74,58,38,0.14); /* hairlines and card borders */
          --gold: #a8843f;         /* the accent — antique gold, dark enough to read */
          --gold-bright: #8a6a35;  /* gold hover states — deeper, not lighter, on paper */
          --gold-soft: #d8b681;    /* decorative gold that need not be read */
  }
...
export default async function Option1() {
  ...
  return (
    <main className="opt1 min-h-screen bg-[var(--paper)] text-[var(--ink)]">
      <style>{PALETTE}</style>
      {/* Hero */}
      <div className="relative overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(70% 50% at 50% 0%, rgba(216,182,129,0.35) 0%, rgba(246,241,231,0) 70%), radial-gradient(40% 30% at 12% 85%, rgba(232,196,190,0.35) 0%, rgba(246,241,231,0) 70%), radial-gradient(35% 30% at 90% 70%, rgba(216,182,129,0.22) 0%, rgba(246,241,231,0) 70%)",
          }}
        />
      ...
      <Section eyebrow="The essentials" title="When and where">
        <div className="mt-10 grid gap-px overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--line)] sm:grid-cols-3">
          {[
            ...
            <div key={d.label} className="bg-[var(--card)] p-7 text-center">
              ...
              {d.sub && <p className="mt-2 text-[13px] text-[var(--muted-2)]">{d.sub}</p>}
            </div>
          ))}
        </div>

        <div className="mt-8 rounded-2xl border border-[var(--line)] bg-[var(--card)] p-8 text-center">
          <p
          ...
          {info.venueAddress && (
            <p className="mt-2 text-[15px] text-[var(--muted)]">{info.venueAddress}</p>
          )}
          {info.mapUrl && (
            <a
              href={info.mapUrl}
      ...
      {evening && evening.items.length > 0 && (
        <Section eyebrow="The evening" title="How the night unfolds">
          <p className="mx-auto mt-4 max-w-xl text-center text-[15px] leading-relaxed text-[var(--muted)]">
          ...
          <ol className="mx-auto mt-12 flex max-w-xl flex-col">
            {evening.items.map((item) => (
              <li
                key={item.id}
                className="grid grid-cols-[80px_1fr] gap-5 border-t border-[var(--line)] py-5 first:border-t-0 sm:grid-cols-[110px_1fr]"
              >
                ...
                <span>
                  ...
                  {item.detail && (
                    <span className="mt-1 block text-[14px] leading-relaxed text-[var(--muted-2)]">
                      {item.detail}
                    </span>
                  )}
                </span>
      ...
      <Section eyebrow="Coming from abroad" title="Travelling to Baku">
        <div className="mt-10 grid gap-6 sm:grid-cols-3">
          {(info.travelInfo
            ...
            <div
              key={c.h}
              className="rounded-2xl border border-[var(--line)] bg-[var(--card)] p-7"
            >
              <h3
              ...
              <p className="mt-3 text-[14px] leading-relaxed text-[var(--muted)]">{c.p}</p>
            </div>
          ))}
        </div>
      </Section>
          ...
          <Section eyebrow="Good to know" title="Questions, answered">
            <div className="mx-auto mt-10 flex max-w-xl flex-col">
              {lines(info.faq).map((l, i) => {
                ...
                return (
                  <div key={i} className="border-t border-[var(--line)] py-5 first:border-t-0">
                    <p className="text-[15px] text-[var(--ink-strong)]">{head}</p>
                    {body && (
                      <p className="mt-1 text-[14px] leading-relaxed text-[var(--muted-2)]">{body}</p>
                    )}
                  </div>
                );
              })}
      ...
      <footer className="border-t border-[var(--line)]">
        <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 px-6 py-12 text-center">
          ...
          {info.contactPhone && (
            <p className="text-[14px] text-[var(--muted-2)]">
